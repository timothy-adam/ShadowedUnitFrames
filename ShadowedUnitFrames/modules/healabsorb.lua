local HealAbsorb = setmetatable({frameKey = "healAbsorb", colorKey = "healAbsorb", frameLevelMod = 2}, {__index = ShadowUF.IncHeal})
...
function HealAbsorb:OnDisable(frame)
	frame:UnregisterAll(self)
	frame.healAbsorb:Hide()
end

function HealAbsorb:UpdateFrame(frame)
	if( not frame.visibility.healAbsorb or not frame.visibility.healthBar ) then return end
